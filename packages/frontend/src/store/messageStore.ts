...
export const useMessageStore = defineStore<string, {
	...
	sendMessage(message: ChatMessage, model: Model, dialogId: DialogId, resendMessageId?: string): Promise<ReadableStream<ChatMessage>>,
	 ...
	 * @returns 消息流
	 */
	const sendMessage = async (userMessage: ChatMessage, model: Model, dialogId: DialogId, resendMessageId?: string) => {
		// 创建用户消息
		userMessage.timestamp = Date.now();
		messages.value[dialogId] = messages.value[dialogId] || [];
		if (resendMessageId) {
			const resetIndex = messages.value[dialogId].findIndex(message => message.id === resendMessageId);
			if (resetIndex > -1) {
		...
		// 调用API发送消息
		const [abort, readableStream] = chatAPI.sendMessage(userMessage, model, dialogId, resendMessageId);
