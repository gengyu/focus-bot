<template>
  <div class="flex" :class="messages.role === 'user' ? 'justify-end' : 'justify-start'">
    ...
    <!-- 消息气泡 -->
    <div class="max-w-[75%] min-w-32 rounded-2xl px-4 py-3 break-all relative group"
    ...
    >
      ...
      <!-- 操作按钮 -->
      <div class="absolute right-2 bottom-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        <button @click="copyMessage" class="p-1 rounded hover:bg-black/10 transition-colors">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
               :class="messages.role === 'user' ? 'text-white' : 'text-gray-500'" fill="none" viewBox="0 0 24 24"
               stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/>
          </svg>
        </button>
        <button v-if="messages.role === 'user'" @click="resendMessage" class="p-1 rounded hover:bg-black/10 transition-colors">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
               :class="messages.role === 'user' ? 'text-white' : 'text-gray-500'" fill="none" viewBox="0 0 24 24"
               stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
